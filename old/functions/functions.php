...
//private $username;
  ...
  public function addUser($username,$password, $fname,$lname,$email, $phone){
		...
		if($checkUser->rowCount() ==1){
			...
			$role = 10; //Admin
				$addUser = $this->dbCon->prepare("INSERT INTO users (username, password, fname,lname,email,phone,role_id) VALUES (:username, :password, :fname,:lname,:email,:phone,:role_id)" );
...
Class Banner{
	...
	public function addBanner($path, $title){
...
//private $username;
	...
	public function getNews(){
		$getNews = $this->dbCon->Prepare("SELECT id,title, content, news_image, date_added FROM news");
		$getNews->execute();
